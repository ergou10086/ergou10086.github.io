class LagrangeBuilder{
    constructor(){
        ...
        // 检测 Butterfly 主题的深色模式
        const themeColorMeta = document.querySelector('html');
        if(themeColorMeta) {
            const observer = new MutationObserver(() => {
                this.toggleDarkMode();
            });
            observer.observe(themeColorMeta, { attributes: true, attributeFilter: ['data-theme'] });
        }
    ...
    initLinearInput(){
        ...
        button.type = "button";
        button.value = "+";
        button.className = "outline secondary lagrange-add-btn";
    ...
    /**
     * 切换深色模式 - 适配 Butterfly 主题
     */
    toggleDarkMode() {
        const htmlElement = document.querySelector('html');
        const isDarkMode = htmlElement && htmlElement.getAttribute('data-theme') === 'dark' ||
                          window.matchMedia('(prefers-color-scheme: dark)').matches;
        
        const darkLink = document.getElementById("darkMode");
        if(isDarkMode){
            if(darkLink && !darkLink.href.includes('dark.css')){
                darkLink.href = "/lagrange/css/dark.css";
            }
        }else{
            if(darkLink){
                darkLink.href = "";
